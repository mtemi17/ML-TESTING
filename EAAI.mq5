...
// --- helper utilities ------------------------------------------------------
double SafeIndicatorValue(double value, double fallback=0.0)
{
                        ...
                        double entryPrice, double stopPrice, double targetPrice)
{
   ...
   if(StringFind(entryType, "LONG") >= 0)
      breakoutDistance = MathMax(closePrice - range.high, 0.0);
   else
      breakoutDistance = MathMax(range.low - closePrice, 0.0);
   ...
   for(int i=1; i<=20; i++)
   {
      double val = iATR(_Symbol, PERIOD_M5, 14, i);
      if(!DoubleIsNaN(val))
      {
         atrSum += val;
         atrCount++;
   ...
   double ema50  = iMA(_Symbol, PERIOD_M5, 50, 0, MODE_EMA, PRICE_CLOSE, 1);
   ...
   string modeValue = entryType;
   string marketValue = _Symbol;
   ...
   payload += StringFormat("\"Mode\":\"%s\",", modeValue);
   payload += StringFormat("\"Market\":\"%s\"", marketValue);
   payload += "} }";
   return payload;
}
