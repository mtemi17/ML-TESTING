...
class TradingStrategy:
    ...
    def backtest_strategy(self):
        ...
        def calculate_breakout_metrics(direction: str, close_price: float, range_high: float, range_low: float, row_data: pd.Series) -> Dict[str, float]:
            candle_range = row_data['High'] - row_data['Low']
            candle_body = abs(row_data['Close'] - row_data['Open'])
            ...
            atr_value = row_data.get('ATR', np.nan)
            ...
            ema_5m = row_data.get('EMA_200_5M', np.nan)
        ...
        for idx, row in self.df.iterrows():
            ...
            high_price = row['High']
            low_price = row['Low']
            ema_value = row.get('EMA_200_1H', np.nan)
            ...
            breakout_up = close_price > range_high
            breakout_down = close_price < range_low

            if state == "idle":
                if config.allow_breakout:
                    if breakout_up and trend_bullish:
                        risk = close_price - range_low
                        if risk > 0:
                            ...
                            entry_info.update(breakout_metrics)
                            state = "in_long"
                            continue
                    elif breakout_down and trend_bearish:
                        risk = range_high - close_price
                        if risk > 0:
